...
- [x] `/zr2 v1 migrate-data` 从 v1 `data/zr_economy.db` 导入硬币/等级/经验/称号/击杀
- [x] 默认跳过已存在玩家，`--overwrite` 强制覆盖
- [x] `/zr2 v1 migrate` 同步导入 v1 `config/doors/*.scandata.yml` 全部门快照到 v2 存储
- [x] 按“方块坐标落在门区域内”最佳匹配自动关联快照到门
- [x] 单测：导入/防覆盖/快照关联
- [x] Paper 实测：3 位玩家 135,443,145 硬币导入成功；10 个 v1 快照导入 v2 存储

## M18：出生点与母体机制定稿

- [x] 母体身份固定：整局保持母体，被击杀不回起点、不降级
- [x] 母体晚释放：`mother-release-delay-seconds`（默认 10s）倒计时前无敌且不能攻击
- [x] 母体死亡/普通僵尸死亡：传回当前 MapFlow 阶段门后的 `DOOR_ZOMBIE` 检查点，3 秒无敌
- [x] 人类非感染死亡：直接淘汰转观战，不再复活
- [x] 人类被感染：转为普通僵尸，从门后检查点继续
- [x] 母体掉线：从普通僵尸中选替补并立即释放
